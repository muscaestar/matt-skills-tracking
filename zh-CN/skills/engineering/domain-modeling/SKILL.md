...
在设计的同时，积极构建和打磨项目的领域模型。这是一种*主动*纪律——挑战术语、发明边界场景，并在它们刚刚成形的瞬间写下术语表和决策。（仅仅*阅读* `CONTEXT.md` 获取词汇不属于本技能——那是任何技能都能做到的一行式习惯。本技能用于你在*改变*模型，而不仅仅是消费模型时。）
...
`CONTEXT.md` 应当完全不包含实现细节。不要把它当作规格、草稿本或实现决策的存放处。它是术语表，仅此而已。
